...
--
include::{generated}/api/defines/XR_NULL_PATH.adoc[]
...
include::{generated}/api/protos/xrStringToPath.adoc[]
...
See <<semantic-paths-path-atom-type>> for the conditions under which an
...
include::{generated}/validity/protos/xrPathToString.adoc[]
--


[[semantic-paths-reserved]]
=== Reserved Paths
...
* pathname:/user/head represents inputs on the user's head, often from a
  ...
  To reason about the user's head, see the <<spaces-reference-spaces,
...
Input subpaths further qualify top-level pathname:/user paths to form
binding paths.
...
pathname:/user path vary by interaction profile.
...
See <<semantic-paths-interaction-profiles, Interaction Profiles>> for
examples of input subpaths.

[[semantic-paths-standard-identifiers]]
...
* thumb_resting_surfaces - Any surfaces that a thumb may naturally rest on.
  This may include, but is not limited to, face buttons, thumbstick, and
  thumbrest (Provided by `XR_VERSION_1_1`)
* stylus - Tip that can be used for writing or drawing.
  May be able to detect various pressure levels (Provided by
  `XR_VERSION_1_1`)
* trigger_curl - This sensor detects how pointed or curled the user's finger
  is on the trigger: 0 = fully pointed, 1 = finger flat on surface (Provided
  by `XR_VERSION_1_1`)
* trigger_slide - This sensor represents how far the user is sliding their
  index finger along the surface of the trigger: 0 = finger flat on the
  surface, 1 = finger fully drawn back (Provided by `XR_VERSION_1_1`)
...
[[semantic-paths-grip_surface-pose]]
* grip_surface - (Provided by `XR_VERSION_1_1`) A pose that allows
  applications to reliably anchor visual content relative to the user's
  physical hand, whether the user's hand is tracked directly or its position
  and orientation is inferred by a physical controller.
  The grip_surface pose is defined as follows:

** The grip_surface position: The user's physical palm centroid, at the
   surface of the palm.
   For the avoidance of doubt, the palm does not include fingers.
** The grip_surface orientation's +X axis: When a user is holding the
   controller and straightens their index fingers pointing forward, the ray
   that is normal (perpendicular) to the user's palm (away from the palm in
   the left hand, into the palm in the right hand).
** The grip_surface orientation's -Z axis: When a user is holding the
   controller and straightens their index finger, the ray that is parallel
   to their finger's pointing direction.
** The grip_surface orientation's +Y axis: orthogonal to +Z and +X using the
   right-hand rule.

image::images/palm_pose.png[align="center", title="Example grip_surface pose for (from left to right) a generic motion controller, tracked hand, and a digital hand avatar. The X axis is depicted in red. The Y axis is depicted in green. The Z axis is depicted in blue."]
...
[[semantic-paths-standard-locations]]
===== Standard locations
...
[[semantic-paths-standard-components]]
===== Standard components
...
* proximity - The user is in physical proximity of input source.
  ...
  The state of a "proximity" component must: be ename:XR_TRUE if the same
  input source is returning ename:XR_TRUE for either a "touch" or any other
  component that implies physical contact.
  The runtime may: return ename:XR_TRUE for "proximity" when "touch" returns
  ename:XR_FALSE which would indicate that the user is hovering just above,
  but not touching the input source in question.
  "proximity" components are always boolean.
  (Provided by `XR_VERSION_1_1`)

[[semantic-paths-output-paths]]
===== Output paths
...
* haptic_trigger - A haptic element located in the trigger (Provided by
  `XR_VERSION_1_1`)
* haptic_thumb - A haptic element located in the resting place of the thumb,
  like under the touchpad (Provided by `XR_VERSION_1_1`)
...
[[semantic-paths-interaction-profiles]]
...
==== ByteDance PICO Neo 3 controller Profile
(Provided by `XR_VERSION_1_1`)

Path: pathname:/interaction_profiles/bytedance/pico_neo3_controller
...
ByteDance PICO Neo3 Controller.
...
==== ByteDance PICO 4 controller Profile
(Provided by `XR_VERSION_1_1`)

Path: pathname:/interaction_profiles/bytedance/pico4_controller
...
==== ByteDance PICO G3 controller Profile
(Provided by `XR_VERSION_1_1`)

Path: pathname:/interaction_profiles/bytedance/pico_g3_controller
...
==== HP Mixed Reality Motion Controller Profile
(Provided by `XR_VERSION_1_1`)
...
This interaction profile represents the input sources and haptics on the HP
...
==== HTC Vive Cosmos Controller Profile
(Provided by `XR_VERSION_1_1`)
...
==== HTC Vive Focus 3 Controller Profile
(Provided by `XR_VERSION_1_1`)

Path: pathname:/interaction_profiles/htc/vive_focus3_controller
...
==== Magic Leap 2 Controller Profile
(Provided by `XR_VERSION_1_1`)

Path: pathname:/interaction_profiles/ml/ml2_controller
...
[[semantic-paths-oculus-touch-controller-profile]]
...
* subpathname:/input/trigger/value
* subpathname:/input/trigger/touch
* subpathname:/input/trigger/proximity (Provided by `XR_VERSION_1_1`)
* subpathname:/input/thumb_resting_surfaces/proximity (Provided by
  `XR_VERSION_1_1`)
* subpathname:/input/thumbstick/x
* subpathname:/input/thumbstick/y
* subpathname:/input/thumbstick/click
...
==== Meta Touch Pro Controller Profile
(Provided by `XR_VERSION_1_1`)

Path: pathname:/interaction_profiles/meta/touch_pro_controller
...
* subpathname:/input/thumbstick/touch
* subpathname:/input/thumbrest/touch
* subpathname:/input/thumbrest/force
* subpathname:/input/stylus/force
* subpathname:/input/grip/pose
* subpathname:/input/aim/pose
* subpathname:/output/haptic
* subpathname:/output/haptic_trigger
* subpathname:/output/haptic_thumb

include::{config}/grip_surface_notice.adoc[]
include::{config}/ext_palm_pose_notice.adoc[]
include::{config}/ext_hand_interaction_notice.adoc[]

==== Meta Touch Plus Controller Profile

(Provided by `XR_VERSION_1_1`)

Path: pathname:/interaction_profiles/meta/touch_plus_controller
...
==== Meta Touch Controller (Rift CV1) Profile
(Provided by `XR_VERSION_1_1`)

Path: pathname:/interaction_profiles/meta/touch_controller_rift_cv1
...
This interaction profile represents the input sources and haptics on the
Oculus Touch controller and is a legacy profile added to specifically
represent the controller shipped with the Rift CV1.
...
==== Meta Touch Controller (Rift S / Quest 1) Profile
(Provided by `XR_VERSION_1_1`)

Path: pathname:/interaction_profiles/meta/touch_controller_quest_1_rift_s
...
This interaction profile represents the input sources and haptics on the
Oculus Touch controller and is a legacy profile added to specifically
represent the controller shipped with the Rift S and Quest 1.
...
==== Meta Touch Controller (Quest 2) Profile
(Provided by `XR_VERSION_1_1`)

Path: pathname:/interaction_profiles/meta/touch_controller_quest_2
...
This interaction profile represents the input sources and haptics on the
Oculus Touch controller and is a legacy profile added to specifically
represent the controller shipped with the Quest 2.
...
It is exactly the same, with the exception of the name of the interaction
profile, as the Microsoft Mixed Reality Controller interaction profile.
It enables the application to differentiate the newer form factor of motion
controller released with the Samsung Odyssey headset.

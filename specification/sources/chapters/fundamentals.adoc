...
Functions with buffer or array parameters passed as pointers, rather than
declared with a static array size, follow different conventions depending on
whether the buffer size is known to the application or variable per call.

==== Variable size buffer parameters
...
the element type being code:float in this case:
...
* If the function returns any etext:XR_ERROR_* result other than
  ename:XR_ERROR_SIZE_INSUFFICIENT, the contents of the values of (or
  pointed to by) pname:elementCountOutput and pname:elements are undefined.
...
* If all pname:elementCapacityInput values are non-zero but **any** is
  ...
  when the function returns any etext:XR_ERROR_* result.

[[fundamentals-known-buffer-size-parameters]]
==== Known size buffer parameters

Functions with known size input and/or output buffer parameters, or buffer
parameters of an application-chosen size, take a slightly different approach
than variable size buffer parameters.
Such functions also take on either parameter form or structure form, as in
...
Unlike for variable size buffer parameters, only a single "count" is
specified per buffer/array.
Functions following this convention have the following behavior with respect
to the array/buffer and its count parameters:

.Buffer Size Parameter Behavior
****
* The pname:elementCount argument precedes the array to which it refers, in
  argument order.
* pname:elementCount specifies the number of elements passed as input, or
  both the capacity in number of elements of the buffer to be written and
  the exact number of elements expected.
* pname:elements must: be at least pname:elementCount elements in size.
* If the array is used as output or input/output and pname:elementCount is
  not equal to the exact number of elements expected, the function must:
  return ename:XR_ERROR_VALIDATION_FAILURE.
* If the array is used as output or input/output and the function returns
  any etext:XR_ERROR_*, including ename:XR_ERROR_VALIDATION_FAILURE, the
  contents of the data in the array pname:elements is undefined.
* If the array is used as input, all pname:elementCount elements of
  pname:elements must: be valid unless otherwise specified.
****

Some functions have a given pname:elementCount associated with more than one
...
call.
For these functions, the pname:elementCount, and pname:elements array
...
the function returns any etext:XR_ERROR_* result.
All of the preceding general requirements continue to apply.
...
* pname:angleLeft is the angle of the left side of the field of view.
  For an asymmetric field of view this value is negative.
...
[open,refpage='XrEventDataBaseHeader',desc='Base header for an event',type='structs',xrefs='xrPollEvent XrEventDataBuffer']
--
...
[open,refpage='XrEventDataEventsLost',desc='Event indicating events were lost',type='structs',xrefs='xrPollEvent XrEventDataBuffer XrEventDataBaseHeader']
--

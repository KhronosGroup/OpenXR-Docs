...
* Independent of pname:elementCapacityInput or pname:elements parameters,
  ...
  parameter form.
* It is valid to pass code:0 for the pname:elementCapacityInput parameter
  (i.e. capacity of 0 does not return ename:XR_ERROR_SIZE_INSUFFICIENT).
  The function sets pname:elementCountOutput to the required size in number
  of elements.
* If code:0 is passed for the pname:elementCapacityInput parameter, the
  pname:elements parameter is ignored (any value passed is considered valid
  usage).
* Where the pname:elementCapacityInput is non-zero but less than required,
  ...
  The data in the array pname:elements is undefined.
* Where the pname:elementCapacityInput is non-zero and the function returns
...
* Upon a failure for reasons unrelated to the element array capacity, the
  contents of the values pointed to by pname:elementCountOutput and
  pname:elements are undefined.
...
--
include::{generated}/api/enums/XR_TRUE.txt[]
--

[open,refpage='XR_FALSE',desc='False enum/define',type='enums',xrefs='XrBool32 XR_TRUE']
--
include::{generated}/api/enums/XR_FALSE.txt[]
--

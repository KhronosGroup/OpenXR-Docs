...
* ename:XR_SWAPCHAIN_USAGE_SAMPLED_BIT indicates that the image may sampled
  by a shader.
...
The runtime must: return ename:XR_ERROR_CALL_ORDER_INVALID if pname:index
...
This layer type is typically used to render the virtual world from the user's
perspective.

...
Runtimes must: not block indefinitely on a second flink:xrWaitFrame call in
the absence of flink:xrBeginFrame/flink:xrEndFrame calls but may do so on
subsequent flink:xrWaitFrame calls.

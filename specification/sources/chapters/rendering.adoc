...
pname:viewConfigurationType that was not passed in the session's call to
flink:xrBeginSession via the
pname:beginInfo.pname:primaryViewConfigurationType, or enabled though an
extension, then the runtime must: return ename:XR_ERROR_VALIDATION_FAILURE.

...
include::../metadata/XR_MSFT_spatial_anchor_persistence.adoc[]
...
The given pname:spatialAnchorName will be the string to retrieve the spatial
...
****
* pname:spatialAnchorStore is the slink:XrSpatialAnchorStoreConnectionMSFT
  with which to persist the pname:spatialAnchor.
...
It is used to perform persist and unpersist on an pname:spatialAnchor in the
...
* pname:spatialAnchorNamesCapacityInput is the capacity of the
...
* See the <<buffer-size-parameters, Buffer Size Parameters>> section for a
  detailed description of retrieving the required pname:persistedAnchorNames
  size.
****
...
If the pname:spatialAnchorName provided does not correspond to a currently
...
The pname:name is a character array of maximum size
...
terminator).
If an empty pname:name value is passed to any function as a parameter, that
function must: return ename:XR_ERROR_SPATIAL_ANCHOR_NAME_INVALID_MSFT.

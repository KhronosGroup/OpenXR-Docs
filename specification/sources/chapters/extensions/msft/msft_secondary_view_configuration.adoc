...
it can support by enabling them during the call to flink:xrBeginSession by
attaching the extension structure
slink:XrSecondaryViewConfigurationSessionBeginInfoMSFT to the
slink:XrSessionBeginInfo::pname:next pointer.
...
* pname:type is the elink:XrStructureType of this structure.
* pname:next is code:NULL or a pointer to an extension-specific structure.
* pname:viewConfigurationType is a elink:XrViewConfigurationType that

...
[open,refpage='XrSystemHandTrackingMeshPropertiesMSFT',type='structs',desc='System property for hand tracking mesh',xrefs='']
--
...
pname:maxHandMeshVertexCount to pre-allocate hand mesh buffers and reuse
them in their render loop when calling flink:xrUpdateHandMeshMSFT every
frame.
...
[open,refpage='xrCreateHandMeshSpaceMSFT',type='protos',desc='Create a space for hand mesh tracking',xrefs='']
--
...
[open,refpage='XrHandMeshSpaceCreateInfoMSFT',type='structs',desc='The information to create a hand mesh space',xrefs='']
--
...
[open,refpage='xrUpdateHandMeshMSFT',type='protos',desc='Update hand mesh buffers',xrefs='']
--
...
The application should: pre-allocate the index buffer and vertex buffer in
...
The application should: pre-allocate the slink:XrHandMeshMSFT structure and
...
[open,refpage='XrHandMeshUpdateInfoMSFT',type='structs',desc='The information to update a hand mesh',xrefs='']
--
...
[open,refpage='XrHandMeshMSFT',type='structs',desc='The data of a hand mesh',xrefs='']
--
...
[open,refpage='XrHandMeshIndexBufferMSFT',type='structs',desc='The index buffer of a hand mesh',xrefs='']
--
...
An application should: pre-allocate the indices array using the
...
[open,refpage='XrHandMeshVertexBufferMSFT',type='structs',desc='The vertex buffer of a hand mesh',xrefs='']
--
...
An application should: pre-allocate the vertices array using the
...
[open,refpage='XrHandMeshVertexMSFT',type='structs',desc='The vertex of hand mesh',xrefs='']
--
...
Following example code demos pre-allocating hand mesh buffers and updating
...
XrHandTrackerEXT leftHandTracker{};
{
    ...
    CHK_XR(xrCreateHandTrackerEXT(session, &createInfo, &leftHandTracker));
}
...
XrSpace leftHandMeshSpace{};
{
    ...
    CHK_XR(xrCreateHandMeshSpaceMSFT(leftHandTracker, &createInfo, &leftHandMeshSpace));
}
...
while(1){
    ...
    CHK_XR(xrUpdateHandMeshMSFT(leftHandTracker, &updateInfo, &leftHandMesh));
...
[open,refpage='XrHandPoseTypeInfoMSFT',type='structs',desc='Describes what hand pose type for the hand joint tracking.',xrefs='']
--
...
[open,refpage='XrHandPoseTypeMSFT',type='enums',desc='Describe type of input hand pose',xrefs='']
--
...
XrHandMeshMSFT referenceHandMesh;   // previously initialized with pre-allocated buffers
...
while(1){
    ...
    CHK_XR(xrUpdateHandMeshMSFT(handTracker, &updateInfo, &referenceHandMesh));
    ...
    if (referenceHandMesh.indexBufferChanged || referenceHandMesh.vertexBufferChanged) {
        ...
        CHK_XR(xrCreateHandTrackerEXT(session, &createInfo, &referenceHandTracker));
        ...
        CHK_XR(xrLocateHandJointsEXT(referenceHandTracker, &locateInfo, &locations));

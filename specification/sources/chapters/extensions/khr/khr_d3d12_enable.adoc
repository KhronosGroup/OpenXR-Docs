...
* The color rendering target image has a memory layout compatible with
  ename:D3D12_RESOURCE_STATE_RENDER_TARGET
* The depth rendering target image has a memory layout compatible with
...
* Having a memory layout compatible with
  ...
  target
* Having a memory layout compatible with
  ename:D3D12_RESOURCE_STATE_DEPTH_WRITE if the image is a depth rendering
  target
...
pointer to an slink:XrGraphicsBindingD3D12KHR in the `next` chain of the
...
the slink:XrGraphicsBindingD3D12KHR.

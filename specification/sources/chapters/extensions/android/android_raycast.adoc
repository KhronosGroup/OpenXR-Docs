...
XrRaycastInfoANDROID rayInfo = {
  .type = XR_TYPE_RAYCAST_INFO_ANDROID,
  .maxResults = NUM_DESIRED_RESULTS,
  .trackerCount = sizeof(trackers) / sizeof(XrTrackableTrackerANDROID),
  .trackers = trackers,
  .origin = headPose.position,
  .trajectory = CalculateForwardDirectionFromHeadPose(headPose),
  .space = appSpace,
  .time = updateTime,
};
...
XrRaycastHitResultsANDROID hitResults = {
  .type = XR_TYPE_RAYCAST_HIT_RESULTS_ANDROID,
  .resultsCapacityInput = NUM_DESIRED_RESULTS,
  .results = hitResult,
};

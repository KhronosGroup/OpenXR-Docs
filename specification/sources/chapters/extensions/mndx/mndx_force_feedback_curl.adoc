...
Submits a request for force feedback for a set of locations. The runtime should: deliver this request to the pname:handTracker device. If the pname:handTracker device is not available, the runtime may: ignore this request for force feedback.

If the session associated with pname:handTracker is not focused, the runtime must: return
ename:XR_SESSION_NOT_FOCUSED, and not apply force feedback.
...
The runtime may: discontinue force feedback if the application that set it loses focus.
An application should: call the function again after regaining focus if force feedback is still desired.

...
restricting physical movement linearly in a single direction.
...
****
* ename:XR_FORCE_FEEDBACK_CURL_LOCATION_FINGER_THUMB_CURL_MNDX specifies
  that force feedback will be applied to the thumb curl
* ename:XR_FORCE_FEEDBACK_CURL_LOCATION_FINGER_INDEX_CURL_MNDX specifies
  that force feedback will be applied to the index finger curl
* ename:XR_FORCE_FEEDBACK_CURL_LOCATION_FINGER_MIDDLE_CURL_MNDX specifies
  that force feedback will be applied to the middle finger curl
* ename:XR_FORCE_FEEDBACK_CURL_LOCATION_FINGER_RING_CURL_MNDX specifies that
  force feedback will be applied to the ring finger curl
* ename:XR_FORCE_FEEDBACK_CURL_LOCATION_FINGER_LITTLE_CURL_MNDX specifies
  that force feedback will be applied to the little finger curl
****
--
...
include::{generated}/api/structs/XrApplyForceFeedbackCurlLocationsMNDX.txt[]
...
Contains an array of slink:XrApplyForceFeedbackCurlLocationMNDX that
contains information on locations to apply force feedback to.

include::{generated}/validity/structs/XrApplyForceFeedbackCurlLocationsMNDX.txt[]
--
...
include::{generated}/api/structs/XrApplyForceFeedbackCurlLocationMNDX.txt[]
...
* pname:value is a value from 0-1 representing the amount of force feedback
  ...
  capable of moving through, with 1 representing making the location incapable
  of movement, and 0 being fully flexible.
...
include::{generated}/validity/structs/XrApplyForceFeedbackCurlLocationMNDX.txt[]
--
...
* pname:locations is an slink:XrApplyForceFeedbackCurlLocationsMNDX
...
the set locations listed in slink:XrApplyForceFeedbackCurlLocationsMNDX.
...
A runtime may: relax force feedback when it sees appropriate when no
application is active, for example on start-up, or if an application has
crashed.
However, the runtime must: keep the last set of location information the
application has passed until the application calls the function again, or

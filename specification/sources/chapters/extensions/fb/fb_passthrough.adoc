...
[open,refpage='XrSystemPassthroughPropertiesFB',type='structs',desc='A passthrough system property',xrefs='XrSystemProperties']
--
...
[open,refpage='XrSystemPassthroughProperties2FB',type='structs',desc='Passthrough system properties',xrefs='XrSystemProperties']
--
...
A single slink:XrSession allows only one slink:XrPassthroughFB.
If an slink:XrPassthroughFB handle has been already created for this
slink:XrSession, the runtime must: return
ename:XR_ERROR_FEATURE_ALREADY_CREATED_PASSTHROUGH_FB.
If an app requires another feature, then the previous one must: be destroyed
by the app with flink:xrDestroyPassthroughFB first.
...
feature handles in progress when the slink:XrPassthroughFB is destroyed.
...
If slink:XrPassthroughFB has not be created for slink:XrSession before
calling this operation, then the runtime must: return
ename:XR_ERROR_FEATURE_REQUIRED_PASSTHROUGH_FB.
...
*Issues*
* Is slink:XrPassthroughFB the parent for slink:XrPassthroughLayerFB?
** From OpenXR conventions, it is clear that the slink:XrSession is the
   parent of slink:XrPassthroughLayerFB, however slink:XrPassthroughLayerFB
   is also strongly associated to slink:XrPassthroughFB and a reasonable
   implementation could result in it be invalidated after
   slink:XrPassthroughFB is destroyed.
** In retrospect, (i.e. if we were going to design this extension again), we
   would explicitly define slink:XrPassthroughFB as the first parameter (and
   therefore parent, instead of slink:XrSession) in
   flink:xrCreatePassthroughLayerFB and thus explicitly define
   slink:XrPassthroughFB as the parent of slink:XrPassthroughLayerFB.
** To avoid potential issues, applications are recommended not to use
   slink:XrPassthroughLayerFB after slink:XrPassthroughFB has been
   destroyed.
   That means that before destroying feature slink:XrPassthroughFB with
   flink:xrDestroyPassthroughFB, an app should destroy all
   slink:XrPassthroughLayerFB handles first, by calling
   flink:xrDestroyPassthroughLayerFB for each such handle.
...
* Revision 5, 2025-01-15 (Ante Trbojevic)
** Explicitly define that only one slink:XrPassthroughFB handle per
   slink:XrSession is allowed.
** Update Issues section with "Is slink:XrPassthroughFB the parent for
   slink:XrPassthroughLayerFB?"
** Change wording "Facebook" to "Meta Platforms"

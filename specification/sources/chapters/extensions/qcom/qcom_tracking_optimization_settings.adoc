...
pfnSetTrackingOptimizationSettingsHintQCOM(session,
    ...
    XR_TRACKING_OPTIMIZATION_SETTINGS_HINT_LONG_RANGE_PRIORITIZATION_QCOM);

// perform tracking while prioritizing long range tracking
----

include::{generated}/interfaces/XR_QCOM_tracking_optimization_settings.adoc[leveloffset=1]
...
* Revision 2, 2025-02-20 (Kurtis Eveleigh)
** Fix typos: rename elink:XrTrackingOptimizationSettingsHintQCOM enum
   values to `PRIORITIZATION`, providing the old names as compatibility
   aliases.

...
* If an application does not include an stext:XrGraphicsBinding* structure
  in the pname:next chain of slink:XrSessionCreateInfo, the runtime must:
  create a "headless" session that does not interact with the display.
...
** Clarify that apps do not need to call `xrGet*GraphicsRequirements` before
   calling `xrCreateSession` when using this extension.

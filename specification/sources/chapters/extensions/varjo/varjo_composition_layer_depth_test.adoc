...
include::{generated}/meta/XR_VARJO_composition_layer_depth_test.adoc[]
...
submission order (as described in <<rendering-compositing>>).
...
estimator (apiext:XR_VARJO_environment_depth_estimation) when enabled.
...
"painter's algorithm" such as described in <<rendering-compositing>> must be
used for layer composition.
...
(see apiext:XR_VARJO_environment_depth_estimation extension) and depth
...
XrCompositionLayerDepthTestVARJO depthTest {
    ...
    .next = layer.next,
    .depthTestRangeNearZ = 0.0f, // in meters
    .depthTestRangeFarZ = 1.0f, // in meters
};
...
include::{generated}/api/structs/XrCompositionLayerDepthTestVARJO.adoc[]
...
include::{generated}/validity/structs/XrCompositionLayerDepthTestVARJO.adoc[]

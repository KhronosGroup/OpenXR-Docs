...
pname:supportsVirtualKeyboard is ename:XR_FALSE when checking the device
compatibility.
...
This function follows the <<buffer-size-parameters,two-call idiom>> for
filling the pname:textureBytes array in the
...
This function follows the <<buffer-size-parameters,two-call idiom>> for
filling the pname:states array in the
...
****
* pname:keyboard is the slink:XrVirtualKeyboardMETA handle.
* pname:info is the slink:XrVirtualKeyboardTextContextChangeInfoMETA

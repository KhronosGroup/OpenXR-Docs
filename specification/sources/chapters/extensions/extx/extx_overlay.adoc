include::../meta/XR_EXTX_overlay.adoc[]
...
indicated in the <<extension>> section.

When you are ready to create an overlay session, you will need to create a
slink:XrSessionCreateInfoOverlayEXTX structure and passing it into the
flink:xrCreateSession via the slink:XrSessionCreateInfo structure's
pname:next parameter.

A runtime will attempt to match or lock the display time between main and
overlay sessions.
If the overlay application's display content is not visually correlated to
features in the main frame (e.g. a HUD or a notification UI) or the overlay
application otherwise does not need its frames' display time to be
synchronized to the main application, the application can set
ename:XR_OVERLAY_SESSION_CREATE_RELAXED_DISPLAY_TIME_EXTX in
pname:createFlags in slink:XrSessionCreateInfoOverlayEXTX.
...
contain the value:
ename:XR_MAIN_SESSION_BIT_ENABLED_COMPOSITION_LAYER_INFO_DEPTH_EXTX.
...
sessions, then the composition will occur in order based on the
overlay session's slink:XrSessionCreateInfoOverlayEXTX::
pname:sessionLayersPlacement value as described in the table below:
...
<<composition-layer-behavior, Composition Layer Behavior>> currently states
that if flink:xrEndFrame is called with 0 layers, then the runtime should
...
ename:XR_MAIN_SESSION_BIT_ENABLED_COMPOSITION_LAYER_INFO_DEPTH_EXTX
ename:XR_OVERLAY_SESSION_CREATE_RELAXED_DISPLAY_TIME_EXTX
...
--
include::{INCS-VAR}/api/structs/XrSessionCreateInfoOverlayEXTX.txt[]

.Member Descriptions
****
* pname:type is the type of this structure.
...
include::{INCS-VAR}/validity/structs/XrSessionCreateInfoOverlayEXTX.txt[]
--
...
--
include::{INCS-VAR}/api/structs/XrEventDataMainSessionVisibilityChangedEXTX.txt[]
...
ename:XR_MAIN_SESSION_BIT_ENABLED_COMPOSITION_LAYER_INFO_DEPTH_EXTX which
indicates if the main session has enabled the
...
include::{INCS-VAR}/validity/structs/XrEventDataMainSessionVisibilityChangedEXTX.txt[]

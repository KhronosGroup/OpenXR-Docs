...
The application uses a <<fundamentals-buffer-size-parameters, two-call
idiom>> with flink:xrGetRenderModelAssetDataEXT to allocate the memory
required for the binary asset data.
...
PFN_xrGetRenderModelStateEXT pfnGetRenderModelStateEXT = nullptr;
CHK_XR(xrGetInstanceProcAddr(
...
XrRenderModelCreateInfoEXT renderModelCreateInfo {
    ...
    .gltfExtensionCount = static_cast<uint32_t>(appSupportedGltfExtensions.size()),
    .gltfExtensions = appSupportedGltfExtensions.data(),
};
CHK_XR(
...
CHK_XR(pfnGetRenderModelPropertiesEXT(renderModel, &propertiesGetInfo, &properties));
...
if (orientationTracked && positionTracked) {
  ...
  XrRenderModelStateEXT state {
      ...
      .nodeStateCount = static_cast<uint32_t>(nodeStates.size()),
      .nodeStates = nodeStates.data(),
  };

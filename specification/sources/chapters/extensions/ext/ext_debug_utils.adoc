include::../meta/XR_EXT_debug_utils.adoc[]
...
This extension provides the following capabilities:
  ...
  - The ability to identify specific OpenXR handles using a
    name to improve tracking.

[[object-debug-annotation]]
==== Object Debug Annotation

It can be useful for an application to provide its own content relative to
a specific OpenXR handle.

[[object-naming]]
===== Object Naming

flink:xrSetDebugUtilsObjectNameEXT allows application
developers to associate user-defined information with OpenXR
handles.

This is useful when paired with the callback that you register when
creating an slink:XrDebugUtilsMessengerEXT object.
When properly used, debug messages will contain not only the
corresponding object handle, but the associated object name as well.
...
structure is an empty string, then any previously set name
is removed.
...
application's use of OpenXR when events of interest occur.
When an event of interest does occur, the debug messenger will submit
a debug message to the debug callback that was provided during
its creation.
Additionally, the debug messenger is responsible with filtering out
debug messages that the callback isn't interested in and will only provide
...
Additionally, each message has a string value identifying its pname:messageId.
...
with both the ename:XR_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT and
ename:XR_DEBUG_UTILS_MESSAGE_TYPE_PERFORMANCE_BIT_EXT flag bits.


===== Message Severity

The severity of a message is a flag that indicates how important the
message is using standard logging naming.
...
.Note
====
The values of elink:XrDebugUtilsMessageSeverityFlagBitsEXT are sorted based on
severity.
...
There are times that it helps to label areas in your OpenXR session
to allow easier debugging.
This can be especially true if your application creates more than
one session.
...
Calls to flink:xrSessionBeginDebugUtilsLabelRegionEXT may be nested allowing you
to identify smaller and smaller labeled regions within your code.
...
--
include::{INCS-VAR}/api/handles/XrDebugUtilsMessengerEXT.txt[]

slink:XrDebugUtilsMessengerEXT represents a callback function and associated filters
registered with the runtime.
--

*New Flag Types*

include::{INCS-VAR}/api/flags/XrDebugUtilsMessageSeverityFlagsEXT.txt[]
include::{INCS-VAR}/api/enums/XrDebugUtilsMessageSeverityFlagBitsEXT.txt[]

include::{INCS-VAR}/api/flags/XrDebugUtilsMessageTypeFlagsEXT.txt[]
include::{INCS-VAR}/api/enums/XrDebugUtilsMessageTypeFlagBitsEXT.txt[]
...
--
include::{INCS-VAR}/api/structs/XrDebugUtilsObjectNameInfoEXT.txt[]

.Member Descriptions
****
* pname:type is the elink:XrStructureType of this structure.
* pname:next is code:NULL or a pointer to an extension-specific structure.
* pname:objectType is an elink:XrObjectType specifying the type of the object to be named.
* pname:objectHandle is the object to be named.
* pname:objectName is a code:NULL terminated UTF-8 string specifying the name to apply to objectHandle.
****

.Valid Usage
****
* If pname:objectType is ename:XR_OBJECT_TYPE_UNKNOWN,
  pname:objectHandle must: not be dlink:XR_NULL_HANDLE
* If pname:objectType is not ename:XR_OBJECT_TYPE_UNKNOWN,
  pname:objectHandle must: be dlink:XR_NULL_HANDLE or
  an OpenXR handle of the type associated with pname:objectType
****
include::{INCS-VAR}/validity/structs/XrDebugUtilsObjectNameInfoEXT.txt[]
--


[open,refpage='XrDebugUtilsLabelEXT',type='structs',desc='Debug Utils Label']
--
include::{INCS-VAR}/api/structs/XrDebugUtilsLabelEXT.txt[]

.Member Descriptions
****
* pname:type is the elink:XrStructureType of this structure.
* pname:next is code:NULL or a pointer to an extension-specific structure.
* pname:labelName is a code:NULL terminated UTF-8 string specifying the label
  name.
****

include::{INCS-VAR}/validity/structs/XrDebugUtilsLabelEXT.txt[]
--

[open,refpage='XrDebugUtilsMessengerCallbackDataEXT',type='structs',desc='Debug utils messenger callback data']
--
include::{INCS-VAR}/api/structs/XrDebugUtilsMessengerCallbackDataEXT.txt[]

.Member Descriptions
****
* pname:type is the elink:XrStructureType of this structure.
* pname:next is code:NULL or a pointer to an extension-specific structure.
* pname:messageId is a code:NULL terminated string that identifies
  the message in a unique way.
  If the callback is triggered by a validation layer, this string corresponds
  the Valid Usage ID (VUID) that can be used to jump to the appropriate
  location in the OpenXR specification.
  This value may: be code:NULL if no unique message identifier is associated
  with the message.
* pname:functionName is a code:NULL terminated string that identifies
  the OpenXR function that was executing at the time the message
...
* pname:message is a code:NULL terminated string detailing the trigger
  conditions.
* pname:objectCount is a count of items contained in the
  pname:objects array.
  This may be 0.
* pname:objects is a pointer to an array of
  slink:XrDebugUtilsObjectNameInfoEXT objects related to the
  detected issue.
...
* pname:sessionLabelCount is a count of items contained in the
  ...
  This may be 0.
* pname:sessionLabels is a pointer to an array of
  slink:XrDebugUtilsLabelEXT objects related to the
  detected issue.
...
include::{INCS-VAR}/validity/structs/XrDebugUtilsMessengerCallbackDataEXT.txt[]
...
.Note
====
This structure should only be considered valid during the lifetime
of the triggered callback.
====

The labels listed inside pname:sessionLabels are organized in time order, with
...
--
include::{INCS-VAR}/api/structs/XrDebugUtilsMessengerCreateInfoEXT.txt[]

.Member Descriptions
****
* pname:type is the elink:XrStructureType of this structure.
* pname:next is code:NULL or a pointer to an extension-specific structure.
* pname:messageSeverities is a bitmask of elink:XrDebugUtilsMessageSeverityFlagBitsEXT
  specifying which severity of event(s) that will cause this callback to be called.
* pname:messageTypes is a combination of elink:XrDebugUtilsMessageTypeFlagBitsEXT
  specifying which type of event(s) will cause this callback to be called.
* pname:userCallback is the application defined callback function to
  call.
...
include::{INCS-VAR}/validity/structs/XrDebugUtilsMessengerCreateInfoEXT.txt[]
...
slink:XrDebugUtilsMessengerCreateInfoEXT::pname:messageTypes determine when that
slink:XrDebugUtilsMessengerCreateInfoEXT::pname:userCallback is called.
...
* The runtime will perform a bitwise AND of the event's
  ...
  slink:XrDebugUtilsMessengerCreateInfoEXT::pname:messageSeverities
  provided during creation of the slink:XrDebugUtilsMessengerEXT object.
...
* The runtime will perform bitwise AND of the event's
  ...
  slink:XrDebugUtilsMessengerCreateInfoEXT::pname:messageTypes
  provided during the creation of the slink:XrDebugUtilsMessengerEXT object.
...
--
include::{INCS-VAR}/api/protos/xrSetDebugUtilsObjectNameEXT.txt[]
...
* pname:nameInfo is a pointer to an instance of the
  slink:XrDebugUtilsObjectNameInfoEXT structure specifying the parameters
...
* In the structure pointed to by pname:nameInfo,
  slink:XrDebugUtilsObjectNameInfoEXT::pname:objectType must:
  not be ename:XR_OBJECT_TYPE_UNKNOWN
* In the structure pointed to by pname:nameInfo,
  slink:XrDebugUtilsObjectNameInfoEXT::pname:objectHandle must:
  not be dlink:XR_NULL_HANDLE
****

include::{INCS-VAR}/validity/protos/xrSetDebugUtilsObjectNameEXT.txt[]
...
--
include::{INCS-VAR}/api/protos/xrCreateDebugUtilsMessengerEXT.txt[]
...
* pname:createInfo points to an slink:XrDebugUtilsMessengerCreateInfoEXT
  structure, which contains the callback pointer as well as defines
  the conditions under which this messenger will trigger the callback.
* pname:messenger is a pointer to which the created
  slink:XrDebugUtilsMessengerEXT object is returned.
****

include::{INCS-VAR}/validity/protos/xrCreateDebugUtilsMessengerEXT.txt[]

The application must: ensure that flink:xrCreateDebugUtilsMessengerEXT is not
...
--
include::{INCS-VAR}/api/protos/xrDestroyDebugUtilsMessengerEXT.txt[]
...
include::{INCS-VAR}/validity/protos/xrDestroyDebugUtilsMessengerEXT.txt[]

The application must: ensure that flink:xrDestroyDebugUtilsMessengerEXT is not
executed in parallel with any OpenXR function that is also called with
pname:instance or child of pname:instance as the dispatchable argument.
--
...
--
include::{INCS-VAR}/api/protos/xrSubmitDebugUtilsMessageEXT.txt[]

.Parameter Descriptions
****
* pname:instance is the debug stream's slink:XrInstance.
* pname:messageSeverity is a single bit value of elink:XrDebugUtilsMessageSeverityFlagsEXT
  severity of this event/message.
* pname:messageTypes is an elink:XrDebugUtilsMessageTypeFlagsEXT bitmask of
  elink:XrDebugUtilsMessageTypeFlagBitsEXT specifying which types of event to
...
.Valid Usage
****
* For each structure in pname:objects found in pname:callbackData,
  the value of slink:XrDebugUtilsObjectNameInfoEXT::pname:objectType
  must: not be ename:XR_OBJECT_TYPE_UNKNOWN
****

include::{INCS-VAR}/validity/protos/xrSubmitDebugUtilsMessageEXT.txt[]
...
userData value that was defined at the time the messenger was
created.
--
...
--
include::{INCS-VAR}/api/protos/xrSessionBeginDebugUtilsLabelRegionEXT.txt[]
...
include::{INCS-VAR}/validity/protos/xrSessionBeginDebugUtilsLabelRegionEXT.txt[]
...
--
include::{INCS-VAR}/api/protos/xrSessionEndDebugUtilsLabelRegionEXT.txt[]
...
.Valid Usage
****
* flink:xrSessionEndDebugUtilsLabelRegionEXT must: be called only after a matching
  flink:xrSessionBeginDebugUtilsLabelRegionEXT.
****

include::{INCS-VAR}/validity/protos/xrSessionEndDebugUtilsLabelRegionEXT.txt[]

This function ends the last label region begun with the
flink:xrSessionBeginDebugUtilsLabelRegionEXT function within
the same pname:session.
--
...
--
include::{INCS-VAR}/api/protos/xrSessionInsertDebugUtilsLabelEXT.txt[]
...
include::{INCS-VAR}/validity/protos/xrSessionInsertDebugUtilsLabelEXT.txt[]
...
--
include::{INCS-VAR}/api/funcpointers/PFN_xrDebugUtilsMessengerCallbackEXT.txt[]

.Parameter Descriptions
****
* pname:messageSeverity indicates the single bit value of elink:XrDebugUtilsMessageSeverityFlagsEXT
  that triggered this callback.
* pname:messageTypes indicates the elink:XrDebugUtilsMessageTypeFlagsEXT specifying
  which types of event triggered this callback.
* pname:callbackData contains all the callback related data in the
  slink:XrDebugUtilsMessengerCallbackDataEXT structure.
...
The callback returns an basetype:XrBool32 that indicates to the calling layer
the application's desire to abort the call.
A value of ename:XR_TRUE indicates that the application wants to abort this
call.
If the application returns ename:XR_FALSE, the function must: not be aborted.
...
to the next element of the slink:XrInstanceCreateInfo structure given
to flink:xrCreateInstance.
...
.editing-note
====
TODO(OpenXR) (i/768): This sample code has testing disabled right now to minimize merge conflicts. It should be re-enabled once !633 is merged.
====
endif::editing-notes[]
...
Associate a name with an slink:XrSpace, for easier debugging in external tools or
...
Labeling the workload with naming information so that any form of
analysis can display a more usable visualization of where actions
occur in the lifetime of a session.
...
You'll notice that "WaitFrame" is no longer available as soon as the
next call to another function like
flink:xrSessionBeginDebugUtilsLabelRegionEXT.
 ...
 ** Clean up some language based on changes going into
    the Vulkan +VK_EXT_debug_utils+ extension by
    Peter Kraus (aka @krOoze).

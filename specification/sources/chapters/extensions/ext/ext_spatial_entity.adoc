...
include::{generated}/api/enums/XrSpatialCapabilityEXT.comments.adoc[]
--
...
[[ext_spatial_entity_capability_features]]
...
include::{generated}/api/enums/XrSpatialCapabilityFeatureEXT.comments.adoc[]
--
...
.Parameter Descriptions
****
* pname:spatialEntity is a handle to an slink:XrSpatialEntityEXT previously
  created by a function such as flink:xrCreateSpatialEntityFromIdEXT.
****
...
* pname:next is code:NULL or a pointer to the next structure in a structure
  chain.
* pname:entityIdCapacityInput is the capacity of the pname:entityIds array,
...
* pname:entityIdCountOutput is the number of basetype:XrSpatialEntityIdEXT
  in pname:entityIds, or the required capacity in the case that
  pname:entityIdCapacityInput is insufficient.
* pname:entityIds is an array of basetype:XrSpatialEntityIdEXT.
  It can: be code:NULL if pname:entityIdCapacityInput is 0.
* pname:entityStateCapacityInput is the capacity of the pname:entityStates
  array, or 0 to indicate a request to retrieve the required capacity.
* pname:entityStateCountOutput is the number of
  elink:XrSpatialEntityTrackingStateEXT in pname:entityStates, or the
  required capacity in the case that pname:entityStateCapacityInput is
  insufficient.
  This must: always be the same as pname:entityIdCountOutput.
...
<<fundamentals-buffer-size-parameters,the two-call idiom>> to retrieve their
data.
...
* pname:bufferCapacityInput is the capacity of the array, or 0 to indicate a
  request to retrieve the required capacity.
* pname:bufferCountOutput is the number of elements in the pname:buffer
  array, or the required capacity in the case that pname:bufferCapacityInput
  is insufficient.
...
The runtime must: not include spatial entities that are not listed in
slink:XrSpatialUpdateSnapshotCreateInfoEXT::pname:entities in
pname:snapshot.
...
XrSpatialContextEXT spatialContext{};
{
  ...
  XrSpatialCapabilityConfigurationFooEXT fooConfig {
    .type = XR_TYPE_SPATIAL_CAPABILITY_CONFIGURATION_FOO_EXT,
    .capability = XR_SPATIAL_CAPABILITY_FOO,
...
auto discoverSpatialEntities = [&](XrSpatialContextEXT spatialContext, XrTime time) {
  ...
  if (completion.futureResult == XR_SUCCESS) {
    ...
    XrSpatialComponentDataQueryConditionEXT queryCond {
      ...
      .componentTypes = &componentToQuery,
    };
...
XrSpatialComponentFooListEXT fooList {
  .type = XR_TYPE_SPATIAL_COMPONENT_FOO_LIST_EXT,
  .fooCount = static_cast<uint32_t>(fooBuffers.size()),
  .foo = fooBuffers.data(),
};
...
for (int32_t i = 0; i < queryResult.entityIdCountOutput; ++i) {
  ...
  if (fooBuffers[i].bufferType == XR_SPATIAL_BUFFER_TYPE_VECTOR3F_EXT) {
    XrSpatialBufferGetInfoEXT getInfo {
      ...
      .bufferId = fooBuffers[i].bufferId,
    };
...
* If an extension defines a new elink:XrSpatialComponentTypeEXT which
...
** The extension must: also define a list structure for that component which
   ...
   Examples of such list structures are
   ...
   use <<fundamentals-buffer-size-parameters,the two-call idiom>> to query
   it, the component data should: provide an slink:XrSpatialBufferEXT for
   each variable-sized data in the list structure and for each such buffer
   it must: specify the elink:XrSpatialBufferTypeEXT.
   Applications can: then query the buffer data using functions defined in
   <<ext_spatial_entity_two_call_component_data>>.
   An example of such a structure is slink:XrSpatialMeshDataEXT which is
   included in slink:XrSpatialComponentMesh3DListEXT.
...
* Extensions can: define structures that extend
  ...
  of the spatial context, but instead provide hints to the runtime on what
  entities and data to include in the snapshot as tracked by the current
  configuration of the spatial context (and therefore the current
  configuration of the underlying services).
* If an extension defines a new elink:XrSpatialCapabilityEXT,
** The extension should: also specify the list of
   elink:XrSpatialComponentTypeEXT that the runtimes must: provide on
   entities for that capability.
** The extension must: also provide structures derived from
   slink:XrSpatialCapabilityConfigurationBaseHeaderEXT that allow the
   configuration of that capability.
* If an extension defines a new elink:XrSpatialCapabilityFeatureEXT,
** The extension must: also define a corresponding configuration structure
   that can: be chained to the next pointer of
   slink:XrSpatialCapabilityConfigurationBaseHeaderEXT, that the application
   can: use to enable the feature when creating an
   slink:XrSpatialContextEXT.
...
* When writing an api that provides the application with a
  basetype:XrSpatialBufferIdEXT, it must: be accompanied by a
  elink:XrSpatialBufferTypeEXT to inform the application of the data type of
  the buffer, so that the application can: use an appropriate
  ftext:xrGetSpatialBuffer* function to retrieve the actual contents of the
  buffer.

[[ext_spatial_entity_semantic_labels_guidelines]]
===== Semantic Label Design & Extension Guidelines

While this extension does not define any component for conveying the
semantic label information of an entity, it lays down the design rules and
guidelines for any extensions that do define such a component.
In the following text, "should:" and "must:" constrain the authors of such
functionality, rather than constraining a runtime implementation or an
application using the API.

* If an extension is defining semantic labels for any
  elink:XrSpatialCapabilityEXT, it should: do so by defining a new
  elink:XrSpatialComponentTypeEXT as well as a new enumeration with a set of
  labels (a "semantic label set") to be the data for that component.
* An enumeration representing a semantic label set must: have a value to
  indicate that the entity is not categorized into any of the labels of this
  set.
  The name of this enumerant must: include 'UNCATEGORIZED', e.g.
  ename:XR_SPATIAL_PLANE_SEMANTIC_LABEL_UNCATEGORIZED_EXT.
  This value indicates to applications that the runtime does recognize this
  entity, but that there are other semantic label sets where this entity is
  categorized.
* Enumeration of this newly defined elink:XrSpatialComponentTypeEXT in
  flink:xrEnumerateSpatialCapabilityComponentTypesEXT implies that the
  runtime supports the semantic label set (and therefore the labels defined
  in the set) corresponding to that component type.
* Applications express their desire to discover entities that have labels
  belonging to this semantic label set by including the corresponding
  elink:XrSpatialComponentTypeEXT in the
  slink:XrSpatialCapabilityConfigurationBaseHeaderEXT::pname:enabledComponents
  array of the configuration struct for the capability that the component
  type is enumerated for.
* It is possible that a single spatial entity has multiple components on it,
  each providing it with a label from their own semantic label set.
  Consider the following example with some hypothetical semantic label sets -
  stext:XrSpatialObjectSemanticLabel, which might have values like chair,
  sofa, table etc., stext:XrSpatialSofaBrand, which might have values like
  IKEA, Wayfair, etc., and stext:XrSpatialIkeaSofa, which might have
  specific IKEA model names of sofas.
  In this scenario, the same sofa entity has all 3 components (labels from
  all 3 semantic label sets) on it, each giving a different granularity of
  information about the entity.
* If an extension is defining a new elink:XrSpatialComponentTypeEXT which
  provides a new semantic label set containing multiple labels in addition
  to the `UNCATEGORIZED` label, the extension should: consider defining an
  elink:XrSpatialCapabilityFeatureEXT which allows applications to activate
  only a subset of the labels defined in the semantic label set
  corresponding to that component type.
  This subset, provided in the next chain of the capability configuration,
  acts as a hint for runtimes to optimize their tracking pipelines and only
  track the subset provided.
  An example of such a feature struct is provided
  <<ext_spatial_entity_example_feature_semantic_label_subset, below>>.
* Extensions must: not add labels to existing semantic label set
  enumeration.
  They must: instead define a new enumeration which contains equivalent
  enumerants with identical semantics and with values equal to those from
  the other set, in addition to the newly added enumerants.
  The specification of the newly defined semantic label set must: also
  establish the mapping between the old label set and the new one.
  See the Spatial Semantic Label Extension example
  <<ext_spatial_entity_example_semantic_label_extension, below>>.

[[ext_spatial_entity_example_feature_semantic_label_subset]]
...
[[ext_spatial_entity_example_semantic_label_extension]]
...
Consider the above example where an extension defines an enumeration
stext:XrSpatialExamplePlaneSemanticLabelExtensionEXT in order to extend the
existing elink:XrSpatialPlaneSemanticLabelEXT with values to represent doors
and windows.
The specification for the new enumeration must: include text of the
following form:

include::../meta/XR_EXT_eye_gaze_interaction.adoc[]
...
This extension provides a basetype:XrPath for getting eye gaze input from an
eye tracker to enable eye gaze interactions.
...
* a basetype:XrPath for real time eye tracking that exposes an accurate and
...
--
include::{INCS-VAR}/api/structs/XrSystemEyeGazeInteractionPropertiesEXT.txt[]
...
include::{INCS-VAR}/validity/structs/XrSystemEyeGazeInteractionPropertiesEXT.txt[]
--
...
poses.
If an slink:XrEyeGazeSampleTimeEXT structure is passed into the
flink:xrLocateSpace call, and while neither pname:space or pname:baseSpace
are bound to an action with toplevel path pathname:/user/eyes_ext/ the
...
since the behaviour can vary considerably for different users and
...
--
include::{INCS-VAR}/api/structs/XrEyeGazeSampleTimeEXT.txt[]
...
include::{INCS-VAR}/validity/structs/XrEyeGazeSampleTimeEXT.txt[]
--

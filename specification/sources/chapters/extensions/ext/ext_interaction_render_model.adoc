...
* See the <<fundamentals-buffer-size-parameters>> section for a detailed
  description of retrieving the required pname:renderModelIds size.
****
...
If any elements in pname:topLevelUserPaths are not valid
<<semantic-paths-user,top level pathname:/user paths>>, the runtime must:
return ename:XR_ERROR_PATH_INVALID from
flink:xrGetRenderModelPoseTopLevelUserPathEXT.
...
for (XrRenderModelIdEXT id : interactionModelIds) {
  ...
  CHK_XR(pfnCreateRenderModelEXT(session, &renderModelCreateInfo, &renderModel));
...
for (XrRenderModelEXT renderModel : interactionModels) {
  ...
  // Get the unique names of the animatable nodes
    ...
    XrRenderModelAssetPropertiesEXT assetProperties {
      ...
      .nodePropertyCount = (uint32_t)nodeProperties.size(),

...
application may: use the event data to adjust the poses accordingly.
...
XrSystemPlaneDetectionPropertiesEXT planeDetectionProperties {
  .type = XR_TYPE_SYSTEM_PLANE_DETECTION_PROPERTIES_EXT,
};
XrSystemProperties systemProperties {
  .type = XR_TYPE_SYSTEM_PROPERTIES,
  .next = &planeDetectionProperties,
};
...
auto processPlanes = [&](const XrTime time) {
    ...
    if (!queryRunning) {
        XrPlaneDetectorBeginInfoEXT beginInfo{
          ...
          .baseSpace = viewSpace,
          .time = time,
          .orientationCount = static_cast<uint32_t>(orientations.size()),

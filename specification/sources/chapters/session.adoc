...
ftext:XrGraphicsBinding* struct of one (and only one) of the enabled
...
(XR_ERROR_VALIDATION_FAILURE may be returned due to legacy behavior) on
calls to flink:xrCreateSession if a function named like

...
Including pathname:/user in the list of subaction paths creates an 'other'
bucket for action data to be filtered into.
When pathname:/user is included in the list passed to
ftext:xrGetActionState* the runtime must: include data for all devices which
are not included in the other subaction paths into the pathname:/user
bucket.
...
of subpathname:/example/path if it is available and apply the same

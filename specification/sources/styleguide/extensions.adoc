...
:uri-extprocess: {uri-openxr-registry-root}/specs/1.1/extprocess.html
...
** Refpages for new functions, structs, enums and enum values, bitfields and
   bits - usually these are ordered roughly in the order they are to be
   used.
   For example:
*** an `XrSystemFooProperties` struct for querying supported features
*** an `xrEnumerateFooCapabilities` function for querying possible
    parameters to create an object with
*** an `XrFoo` handle definition
*** an `xrCreateFoo` function for creating an object
*** an `xrDestroyFoo` function for destroying an object may be put directly
    under its respective `xrCreateFoo` function, or later towards the end
    depending on most useful organization
*** after each function, refpages for each new struct type that appears in
    the arguments of the preceding function such as `XrFooCreateInfo`.
    After those structs, refpages for structs that can be chained to these
    structs.
** Example code - if any
** A list of `New Object Types`, `New Enum Constants`, etc.
   is automatically generated and to be included as an asciidoc include
...
\include::{generated}/interfaces/XR_KHR_foo.adoc[leveloffset=1]
...
* The **direction** may be specified to indicate a negative value
  (`dir="-"`) when needed for negative etext:XrResult values indicating
  errors, like etext:XR_ERROR_ANDROID_THREAD_SETTINGS_FAILURE_KHR.
...
See also
https://www.khronos.org/registry/OpenXR/specs/1.1/extprocess.html#bitmasks[the

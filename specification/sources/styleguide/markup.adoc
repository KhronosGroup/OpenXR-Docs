// Copyright (c) 2015-2020 The Khronos Group Inc. This work is licensed under a
...
The standardized way of referring to an extension by name is to wrap it in
both backticks and angle-brackets: for example, `<<XR_KHR_vulkan_enable>>`.
When a link would be inappropriate (e.g. in the "`Name`" portion of the
extension's spec itself), surrounding it with backticks only is acceptable:
...
----
Extensions, like commands and structures, have an auto-generated anchor.
The standardized way of referring to an extension by name is to wrap it
in both backticks and angle-brackets: for example, `<<XR_KHR_vulkan_enable>>`.
When a link would be inappropriate (e.g. in the "`Name`" portion of the
extension's spec itself), surrounding it with backticks only is acceptable:
`XR_KHR_vulkan_enable`.
----
...
defined by the <<openxr-spec,OpenXR API Specification>> and Reference Page
configuration files.
...
| tlink{cl}     | Generates a cross-reference or link to the definition
                  ...
                  tlink{cl}PFN_xrCreateInstance ->
                  tlink:PFN_xrCreateInstance. This is only used for
                  function pointer types at present.
| tname{cl}     | Formats the macro argument like tlink{cl}. Does not
                  generate a cross-reference. Example:
                  tname{cl}PFN_xrCreateInstance ->
                  tname:PFN_xrCreateInstance.
...
.editing-note
====
TODO(OpenXR): We don't nicely unify our macros like this suggests.
Should we? Otherwise, should we reword this to list all sections?
====
endif::editing-notes[]
...
| dlink{cl}        | Generates a cross-reference or link to the definition of
                     ...
                     "`e`" appendix of the <<openxr-spec,OpenXR
                     API Specification>>
...
| basetype{cl}     | Formats the macro argument like a basic scalar type
                     ...
                     basetype{cl}XrSize -> basetype:XrSize.
...
appendix in the <<openxr-spec,OpenXR API Specification>>.
...
<<openxr-spec,OpenXR API Specification>>, and is used to visually tag terms
which express mandatory and optional behavior of OpenXR implementations, and
of applications using OpenXR.

Whenever one of these terms appears in the <<openxr-spec,OpenXR API
...
.editing-note
====
TODO(OpenXR): Evaluate vkBegin and vkEnd below and replace appropriately or
remove the rule.
====
endif::editing-notes[]

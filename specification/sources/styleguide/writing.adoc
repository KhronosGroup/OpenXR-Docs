...
Note that while some text still calls these extension structures, it is more
accurate to call them chained structures, since there are cases where
structures are chained entirely within the core specification.
Wording in the spec conflating chained structures and extensions will be
gradually corrected over time.
